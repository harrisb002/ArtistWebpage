...
void Artists::writeHTMLFile(std::string htmlString) {
    ...
    std::fstream inputFile("HTML_Master.html");

    if (!inputFile.is_open()) {
        std::cerr << "Failed to open file: " << "HTML_Master.html" << std::endl;
        exit(6);
    }
    ...
    if (pos == std::string::npos) {
        std::cerr << "Placeholder not found in file: " << "HTML_Master.html" << std::endl;
        exit(7);
    }
    ...
    std::fstream outputFile("HTML_Master.html");
